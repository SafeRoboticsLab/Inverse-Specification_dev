...
def main(args):
  ...
  if args.no_cluster:
    fig1, fig2, fig3 = plot_pop_swri(
        ...
        component_values_bound, c='b', alpha=0.5, show_legend=False
    )
  else:
    fig1, fig2, fig3 = plot_pop_swri(
        features_plotted, scores, component_values, objective_names,
        input_names_dict, objectives_bound_plotted, scores_bound,
        component_values_bound, c=labels, alpha=0.5, show_legend=True,
  ...
  if args.no_cluster:
    fig4 = plot_single_objective(
        predicted_scores, dict(o1="Predicted Scores"), axis_bound=None, c='b',
        alpha=0.8
    )
  else:
    fig4 = plot_single_objective(
        predicted_scores, dict(o1="Predicted Scores"), axis_bound=None,
        c=labels, alpha=0.8
    )
  fig4.tight_layout()
  ...
  if args.no_cluster:
    scatter = ax.scatter(
        scores.reshape(-1), predicted_scores.reshape(-1), c='b', alpha=0.8,
        s=sz
    )
  else:
    scatter = ax.scatter(
