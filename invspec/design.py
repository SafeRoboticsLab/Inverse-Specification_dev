...
class Design():
  ...
  def __init__(
      ...
      test_results: Mapping[str, Mapping[str, np.ndarray]], design_id: Any
  ) -> None:
    self.design_param = DesignParameters(physical_components, test_params)
    self.fwd_features = ForwardFeatures(test_results)
    self.id = design_id
...
class DesignParameters():
  ...
  def __init__(
    ...
    Args:
        physical_components (dict): key can be "components" or "graph".
